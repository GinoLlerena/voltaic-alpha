...
| `EXIT-013` | **Substantially closed 31 August 2026.** A credentialed worker with hosted PostgreSQL, a single-writer lease, startup reconciliation and migrations is deployed on a host separate from the public dashboard, with forced-restart evidence. Monitoring, backup and restore are now done: hourly dumps each verified by restoring them, and a five-minute watchdog that fails closed and raises a durable incident. `EXIT-AC-16` is met except that no notification channel is configured, so failures are recorded and displayed rather than pushed, and credential rotation is undocumented. | `EV-024`, `EV-033`, runbook sections 7 and 10 |
