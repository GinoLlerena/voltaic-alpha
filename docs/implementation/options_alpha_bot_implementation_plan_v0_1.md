...
| Per-trade risk | The design proposes approximately 0.75% standard and 1.0% hard cap; the fixture used 0.5% until 31 August 2026, when live pricing showed 0.5% could not fund the smallest structure the delta bands produce. Standard risk is now 0.75%, matching the design, under `POLICY_VERSION` `h0-provisional-1`. | Remaining: set an explicit hard ceiling against confirmed contest equity/scoring. The 0.60 debit/width screen is now the binding constraint and is the threshold most in need of sensitivity evidence. See `DEC-010`. |
...
| 2026-08-28 | Treat the demo host as a constrained decision (`D-01`) rather than an open one | The platform pages read as mandating Streamlit, Replit, or Vercel, none of which hosts an always-on Python worker well, so the judge views and the decision worker could not be assumed to share a runtime | `HK-009`, `HK-011`, `DEC-005`, `OPS-004`, `SRC-UI` |
| 2026-08-29 | Close `D-01`: organizers confirmed no deployment-platform restriction | The separation it forced is kept anyway. A read-only judge view over produced evidence, with the credentialed worker on its own host, is the right architecture on its merits and not merely a way to satisfy a rule that turned out not to exist | `HK-009`, `HK-011`, `DEC-005` |
