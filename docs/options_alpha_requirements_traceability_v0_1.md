...
| `CLR-019` | Current configuration disables trading; plan includes paper execution; autonomy/human-approval rules are unknown. | Fallback `F-04` is binding: build the autonomous end-to-end path with the operator approval boundary behind explicit configuration (`REQUIRE_OPERATOR_APPROVAL`, default `true`, recorded on every decision); the judge demo runs the autonomous configuration. Proof remaining: freeze allowed bot modes, credential permissions, and demo behavior before write access. | Product, Trading, Backend | Phase 1 | `PARTIAL` |
...
| `HK-003` | Confirm the required Alpaca Paper account type, starting equity, reset policy, account age, and credential-verification process. Proof: eligible account ID recorded without secrets. | `SRC-EVENT`, `SRC-DESIGN` | P0 | Product, Trading | `PARTIAL` | Public page confirms Paper use and current credentials pass connectivity; starting equity, reset/account-age rules, options level, and contest eligibility remain. Fallback `F-05` binding: no constant equity exists anywhere; sizing derives only from a fresh timestamped account snapshot, so any starting equity or reset is correct by construction. |
...
| `HK-007` | Confirm whether fully autonomous execution is required or whether human approval is permitted. Proof: selected bot mode and demo behavior match rules. | `SRC-EVENT` | P0 | Product, Trading | `NEEDS_CONFIRMATION` | Current repository intentionally has no execution. Fallback `F-04` binding: the autonomous path is built and the approval boundary is a configuration value, satisfying either answer without a rewrite. |
...
| `HK-009` | Provide a working prototype that judges can use online. Proof: unauthenticated or judge-authenticated hosted demo smoke test. | `SRC-LABLAB` | P0 | DevOps, Frontend | `PENDING` | Hosting is constrained to Streamlit, Replit, or Vercel. No application or hosting configuration exists; the platform choice is open decision `D-01` in `SRC-PHASE0` §6a, owed by the start of Phase 5. |
| `HK-010` | Provide a public GitHub repository with reproducible setup and run instructions. Proof: public URL works in a clean session. | `SRC-LABLAB` | P0 | DevOps | `PENDING` | A public repository is **mandatory**; a private one "may lower your overall score". `github.com/GinoLlerena/voltaic-alpha` may be created private but must be public before submission. Flipping it public is a `G4` checklist item. |
...
| `HK-015` | Provide a 16:9 cover image and required media assets. Proof: final asset renders on the submission preview. | `SRC-LABLAB` | P1 | Presentation | `PENDING` | Confirmed 28 August 2026: PNG or JPG, 16:9. Create after product name and dashboard visuals stabilize. |
...
| `DEC-006` | Does the team have OPRA access, or must P0 rely on the indicative option feed? | `CLR-015` | `G1` | Trading, Backend | Open; fallback `F-06` binding (assume indicative) |
